...
if _HAS_TENSORFLOW:
    ...
    _TENSORFLOW_TYPE = [TensorflowArray]

    if _HAS_JAX:
        @_to_jax.register(TensorflowArray)
        def _(tensor, dtype=None):
            return asarray_jax(tensor.numpy(), dtype=dtype)
else:
    _TENSORFLOW_TYPE = []

# Currently support backends if the Python respective packages are installed
_ARRAY_TYPES = tuple([ndarray] + _JAX_TYPE + _TENSORFLOW_TYPE + [list])
_SUPPORTED_BACKENDS = ['numpy', 'jax', 'tensorflow']
...
def asarray(tensor, dtype=None, backend=None):
    ...
    val, _ = _asarray_helper(tensor, dtype=dtype, backend=backend)
    return val


def _asarray_helper(tensor, dtype=None, backend=None):
    """Convert input to a tensor on the specified backend."""
    if backend == 'numpy' or (
            backend is None and isinstance(tensor, (ndarray, list))):
        return asarray_numpy(tensor, dtype=dtype), 'numpy'

    if _HAS_JAX and (backend == 'jax' or (
            backend is None and isinstance(tensor, JaxArray))):
        return _to_jax(tensor, dtype=dtype), 'jax'

    if _HAS_TENSORFLOW and (backend == 'tensorflow' or (
            backend is None and isinstance(tensor, TensorflowArray))):
        return convert_to_tensor(tensor, dtype=dtype), 'tensorflow'
